...
for (i in loopIndex){
  ...
  load(con)
  if (i==loopIndex[1]) all.data <- model.svm3.sweep$results
  else all.data <- rbind(all.data, model.svm3.sweep$results[-1,])
  close(con)
}
...
p <- ggplot(all.data, aes(x = C, y = Accuracy)) + 
  ...
  ggtitle('SVM with 2nd degree polynomial function basis')
print(p)
